...
/**
 * Menu model used by the app header menu and nested submenus.
 * Each item is identified by a stable `anchor` and may contain children in `menu`.
 */
...
/**
 * Pinia store that holds the application menu tree and provides helpers to:
 * - extract shortcuts (all / executable-only)
 * - update, create, and delete menu items by `anchor`
 */
        ...
        ChangeElementState(
            ...
            if (foundItem) {
                ...
                for (const key in changeItems) {
                    if (
                        Object.prototype.hasOwnProperty.call(changeItems, key)
                    ) {
